...
string inFlightNo;
	...
	case 5: {
		ofstream theFile;
			theFile.open("newFlight.txt", ios::app);
			
			theFile.open("newFlight.txt", std::ofstream::out | std::ofstream::trunc);//Clear current content of the text file
				
				for(int i=0; i<size ; i++){
			    theFile <<obj[i].m_flightNo<<endl;
				theFile <<obj[i].m_depTime<<endl;
				theFile <<obj[i].m_depAir<<endl;
				theFile <<obj[i].m_arrAir<<endl;
				
				for(int j=0; j<obj[i].rows ; j++){

					theFile<<obj[i].m_seat[j].row_Num<<" ";
					theFile<<obj[i].m_seat[j].seat_class<<" ";
					theFile<<obj[i].m_seat[j].seat;
