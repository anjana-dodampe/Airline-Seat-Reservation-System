...
//					cout<<"\t"<<"\t"<<"\t"<<m_seat[j].seat<<endl;
			...
			//Deleting Selected Seat from the array
			...
			void exitSaving(ofstream &theFile ){
				
			//	fin <<"Hello";
				
			    theFile <<m_flightNo<<endl;
				theFile <<m_depTime<<endl;
				theFile <<m_depAir<<endl;
				theFile <<m_arrAir<<endl;
				
				for(int j=0; j<rows ; j++){

					theFile<<m_seat[j].row_Num<<" ";
					theFile<<m_seat[j].seat_class<<" ";
					theFile<<m_seat[j].seat;
					theFile<<"\n";
...
string inFlightNo;
	...
	case 4 :{
		...
		ofstream theFile;
			...
			//theFile.open("testFlight.txt", std::ofstream::out | std::ofstream::trunc);//Clear current content of the text file
			theFile.open("testFlight.txt", ios::app );
			
			//theFile <<"Hellop";
			
				for(int i=0; i<size ; i++)
	       {
		    obj[i].exitSaving( theFile);
	       }
	       break;
